...
with $1000$ rounds, and averaged over multiple trials.  Please refer to Figure $insert$ for more information about the specific parameters used.  We found that the choice of parameters
were roughly equal when tuning them on the other distributions.
...
\texttt{greedyPoker} performed the best on the three datasets, acheiving nearly a third of the expected regret as \texttt{Poker}.  On the other hand, \texttt{Poker} performed second-best
...
the Poker algorithm signficantly outperformed the other algorithms, including \texttt{boltzPoker} by greater than a factor of 5. Later, we were able to tune the parameters
